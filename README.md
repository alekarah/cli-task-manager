# Менеджер Задач - Пет-проект

Персональный менеджер задач, реализованный на двух языках программирования: **Go** и **Python**.

## Описание

Это CLI-приложение для управления личными задачами с возможностью:
- Создания новых задач
- Редактирования существующих задач
- Изменения статуса задач
- Просмотра списка всех задач
- Фильтрации задач по статусу
- Удаления задач

Данные сохраняются в JSON формате для удобства и переносимости.

## Структура проекта

```
cli-task-manager/
├── Go/              # Реализация на Go
│   ├── main.go
│   ├── task.go
│   ├── storage.go
│   ├── go.mod
│   └── README.md
│
└── Python/          # Реализация на Python
    ├── main.py
    ├── task.py
    ├── storage.py
    └── README.md
```

## Запуск проектов

### Go версия
```bash
cd Go
go run .
```

### Python версия
```bash
cd Python
python main.py
```

## Функционал

### Статусы задач
- 📋 **todo** - задача к выполнению
- ⚙️ **in_progress** - задача в процессе выполнения
- ✅ **done** - задача выполнена

### Доступные команды
1. `list` - просмотр всех задач
2. `add` - добавление новой задачи
3. `edit` - редактирование задачи
4. `status` - изменение статуса задачи
5. `delete` - удаление задачи
6. `filter` - фильтрация задач по статусу
7. `exit` - выход из программы

## Особенности реализации

### Go версия
- Использует стандартную библиотеку Go
- Строгая типизация
- Структуры и методы
- Эффективная работа с JSON

### Python версия
- Использует стандартную библиотеку Python
- ООП подход с классами
- Type hints для лучшей читаемости
- Pythonic код

## Возможные улучшения

- [x] Фильтрация задач по статусу
- [ ] Сортировка задач (по дате, приоритету)
- [ ] Добавление приоритетов задач
- [ ] Добавление тегов/категорий
- [ ] Установка дедлайнов
- [ ] Поиск по задачам
- [ ] Экспорт в другие форматы (CSV, Markdown)
- [ ] Web-интерфейс
- [ ] REST API
- [ ] База данных вместо JSON

## Технологии

- **Go**: 1.x+
- **Python**: 3.7+

## Лицензия

Свободный пет-проект для обучения и демонстрации навыков программирования.
